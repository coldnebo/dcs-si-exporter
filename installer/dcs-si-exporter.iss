...
const
  ExportLuaLine = 'pcall(function() local silfs=require(''lfs'');local mod_dir = silfs.writedir()..[[Mods\Services\SayIntentionsExport\]];_G.SIEXPORT_BASE_DIR = mod_dir;dofile(mod_dir..[[SayIntentionsExport.lua]]); end,nil)';
...
// Function to ensure a single trailing backslash
...
begin
  ...
  ConfigContent[3] := '  sayintentions_path = [[' + EnsureTrailingBackslash(SayIntentionsPath) + ']],';
  
  if RealWeatherPath <> '' then
    RealWeatherLine := '  realweather_path = [[' + EnsureTrailingBackslash(RealWeatherPath) + ']]'
  else
    RealWeatherLine := '  realweather_path = nil';
  
  ConfigContent[4] := RealWeatherLine;
  ConfigContent[5] := '}';
